...
class TrainApp():
    ...
    def get_matches(self, top_k=10):
        ...
        curr_sorted_matches = {k: v for k, v in sorted(matches.items(), key=lambda item: item[1], reverse=True)}
        ...
        cleaned_sorted_matches = {k: v for k, v in sorted(cleaned_sorted_matches.items(), key=lambda item: item[1], reverse=True)}
    ...
    def update_curr_match_status(self):
        ...
        self.logger.critical(f"curr_sorted_matches----------->{self.curr_sorted_matches}")
        ...
        if self.golden_matches is not None:
            ...
            self.logger.critical(f"top_ground_truth_matches----------->{top_ground_truth_matches}")
