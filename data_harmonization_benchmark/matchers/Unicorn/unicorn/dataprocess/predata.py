...
class InputFeatures(object):
    ...
    def __init__(
        self, input_ids, input_mask, segment_ids, label_id, exm_id, task_id=-1
    ):
            ...
            for i, tuple in enumerate(pair):
                if sep_token in tuple:
                    (
                        ...
                        input_mask,
                        segment_ids,
                    ) = convert_one_example_to_features_sep(
                        tuple,
                        max_seq_length,
                        cls_token,
                        sep_token,
                    ...
                    (
                        ...
                        input_mask,
                        segment_ids,
                    ) = convert_one_example_to_features(
                        tuple,
                        max_seq_length,
                        cls_token,
                        sep_token,
            ...
            for i, tuple in enumerate(pair):
                if sep_token in tuple:
                    (
                        ...
                        input_mask,
                        segment_ids,
                    ) = convert_one_example_to_features_roberta_sep(
                        tuple,
                        max_seq_length,
                        cls_token,
                        sep_token,
...
def convert_one_example_to_features_sep(
    tuple, max_seq_length, cls_token, sep_token, pad_token, tokenizer
):
...
def convert_one_example_to_features_roberta_sep(
    tuple, max_seq_length, cls_token, sep_token, pad_token, tokenizer
):
    left = tuple.split(sep_token)[0]
    right = tuple.split(sep_token)[1]
...
def convert_fea_to_tensor_one_tuple(features_list, batch_size, do_train):
    ...
    dataset = TensorDataset(
        all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_exm_ids
    )
    sampler = SequentialSampler(dataset)
    if do_train == 0:
        dataloader = DataLoader(dataset, sampler=sampler, batch_size=batch_size)
